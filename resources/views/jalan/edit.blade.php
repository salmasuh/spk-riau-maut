    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div>
        ...
        <select name="status" class="w-full p-3 border rounded" required>
          ...
          <option value="Tidak Aktif" {{ old('status', $jalan->status) == 'Tidak Aktif' ? 'selected' : '' }}>Tidak Aktif</option>
        </select>
      </div>
      ...
      <div class="md:col-span-2">
        ...
        <input
          ...
          value="{{ old('tanggal_input', isset($jalan->tanggal_input) ? \Carbon\Carbon::parse($jalan->tanggal_input)->format('Y-m-d') : now()->format('Y-m-d')) }}"
          max="{{ date('Y-m-d') }}"
          required>
